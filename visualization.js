// Artist's statement: I wanted to provide many different viewpoints on the concept of marriage and widowhood, since different cultural perspectives towards
// widows is something that has been very apparent to me growing up in Indian, British, and American contexts. It's interesting how different the treatment towards
// widows is in different places. Many times in Britain, we see that 

// Color: Since the overall mood is more somber, I had a black background. Still, I wanted to use muted colors for the different components of the 
// data visualizatoins, since would not be overly cheerful, but could still add visual interest and show the differences between countries.

// Typography: I tried to stick to simpler fonts (just 2 this time!) but make sure that the sizing & color indicated visual hierarchy, and make sure it was all aligned.

// Interactivity: I had a lot of fun playing around with different ways to add interactivity, including creating the scrollytelling mechanism (so hard!)
//, adding pop-ups to the flowers, and creating the "guessing" game at the end. I especially spent a lot of time configuring this guessing game, so that the shapes
// and colours were intuitive, so that the animation was "revealing", and so that the prompts responded to user input.

// Data Credits: World Widows Report for data
...
// I did all my data analysis in my ipynb which I submitted as well!! 
...
let heights = [286.0,57.5,325.0,166.0,249.0,100.0,318.5,227.5,78.5,309.5,150.5,221.5,290.0,200.0,338.0,111.0,157.0,325.5];
...
let pyramidWidth = 150; // Width of a single pyramid
let pyramidHeight = 240; // Height of a single pyramid
...
let barData= [84.7,187.65,97.45,138.95,14.8,39.7,19.2,44];
...
function setup() {
  ...
  for (let i = 1; i < barData.length; i += 2) {
    ...
    let sliderY = 875; // Place slider directly below the bar graph
    let slider = createSlider(0, 200, 50);
...
checkButton.position(canvasWidth * 0.95, canvasHeight * 0.12);
...
checkButton.mousePressed(checkSliders);
  ...
  for (let pop of populations) {
    ...
    let size = map(scaledLog, minLog, maxLog, 50, 150); // p5.js map function
    sizes.push(size);
  }
	// Flowers 
...
activeCategories = [true, true, true, true, true]; 
  ...
  fill(255, fadeAlpha);
  drawBarGraph(150, -250, canvasWidth - 300, 1050); // Draw the bar graph
  ...
  pop();
		...
		textFont('Courier New');
		text("Use the sliders to estimate the mortality rates for widowed individuals ", width/30, height / 15 + 85);
		textSize(10 * ratio);
		text("240*", width/30 , height/2 -50);
		text("180*", width/30 , height/2 +50);
		text("120*", width/30 , height/2 +150);
		text("60*", width/30 , height/2 + 250);
		text("*Deaths per 100,000 individuals", width/30, height*0.9 + 75);
		fill(150);
		if (!showComparison){
			 textSize(15 * ratio);
				text("The solid white bars are the true mortality values for married \nindividuals, but the dashed grey lines are for you to guess. ", width * 0.4, height / 15 + 160);
		}
		else{
			if (score > 20){
				textSize(15 * ratio);
				text("Ah, good try but not quite!... ", width * 0.5, height / 15 + 160);
			}
			else{
				textSize(15 * ratio);
				text("Not bad! Have you done this before?\nIt's pretty depressing, no?", width * 0.5, height / 15 + 160);
			}
			textSize(15 * ratio);
			text("The solid white bars are the true mortality values\nfor married individuals, the dashed are your guess, \nand the red is the true mortality for widowed.", width * 0.5, height / 15 + 270);
		}
...
function drawPyramids(countryData, yStart, pyramidWidth, pyramidHeight, horizontalSpacing) {
  let x = 250; // Initial horizontal position
  let y = 400;
